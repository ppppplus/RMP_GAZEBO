...
class robot:
    ...
    def __init__(self, name):
        ...
        # rospy.loginfo("debug:%s-%s",self.global_frame, self.name+'/'+self.robot_frame)
        self.listener.waitForTransform(
            self.global_frame, self.name+'/'+self.robot_frame, rospy.Time(0), rospy.Duration(10.0))
        cond = 0
        while cond == 0:
            try:
                rospy.loginfo('Waiting for the robot transform from %s to %s',self.global_frame, self.name+'/'+self.robot_frame)
                (trans, rot) = self.listener.lookupTransform(
                    self.global_frame, self.name+'/'+self.robot_frame, rospy.Time(0))
                rospy.loginfo("robot transform get!")
                cond = 1
        ...
        rospy.loginfo("robot init:%s!",self.name)
    ...
    def sendGoal(self, point):
        ...
        rospy.loginfo('robot: %s  x: %f y: %f' %(self.name, point[0],point[1]))
